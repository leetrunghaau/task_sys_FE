...
import {
    ...
    createMultipleChecklists,
    updateChecklistItem,
    deleteChecklistItem,
} from "../../../../services/API/checkListAPI";
import { useState, useEffect } from "react";
import { Box, Button, Flex, Heading, HStack, Input, Checkbox, Text, VStack, useToast } from "@chakra-ui/react";
    ...
    const addCheckListSubmit = async (value) => {
        if (value.trim()) {
            try {
                ...
                const addedCheckList = await createSingleChecklist(pid, id, newCheckList);
                fetchCheckLists()
                setAddCheckList(null);
            } catch (error) {
    ...
    const handleDeleteCheckList = async (checkListId) => {
        try {
            ...
            toast({
                ...
                description: "There was an issue deleting the CheckList. Please try again.",
                status: "error",
                duration: 3000,
                isClosable: true,
            });
    ...
    return (
        <Box mt={6}>
            <Flex direction="row" align="start">
                ...
                <SquarePlus onClick={addCheckListClick} />
            </Flex>
            <VStack align="start" spacing={4}>
                {checkLists.map((checkList) => (
                    ...
                    >
                        ...
                        onChange={(e) => {checkedClick(checkList.id, e.target.checked)}}
                        />
                        <EditLine
                            size="sm"
                            area={true}
                ...
                {addCheckList ?
                    <AddLine
                        ...
                        onFinish={(value) => { addCheckListSubmit(value) }}
                    /> : <></>}
            </VStack>
        </Box>
    )
